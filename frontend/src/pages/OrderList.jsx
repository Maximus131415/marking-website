...
export default function OrderList() {
  ...
  const takeOrder = async (orderId) => {
    try {
      ...
      if (res.ok) {
        // Переходим к интерфейсу разметки внутри того же приложения
        navigate(`/marking?orderId=${orderId}`);
      } else { 
        alert("Заказ уже недоступен!"); 
        fetchOrders(); 
      }
    } catch (e) { 
      console.error(e); 
  ...
  const downloadResults = async (orderId) => {
    try {
      ...
      if (res.ok) {
        ...
        a.href = url; 
        a.download = `results_${data.order_title}.json`; 
        a.click();
      } else {
        alert("Ошибка выгрузки результатов");
      }
    } catch (e) { 
      console.error(e); 
    }
  ...
  return (
    <div className="container" style={{ maxWidth: '900px' }}>
      <button className="btn btn-outline" onClick={() => navigate('/dashboard')} style={{ marginBottom: '20px' }}>
        ⬅ Назад в кабинет
      </button>
      ...
      <div style={{ display: 'grid', gap: '15px' }}>
        {orders.length === 0 ? ( 
          <div className="card" style={{ textAlign: 'center' }}>Пока здесь пусто.</div> 
        ) : (
          orders.map(order => (
            <div key={order.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ width: role === 'customer' ? '60%' : 'auto' }}>
                ...
                {role === 'customer' && (
                  <div style={{ background: 'var(--bg-primary)', height: '12px', borderRadius: '6px', overflow: 'hidden', position: 'relative', marginTop: '10px' }}>
              ...
              {role === 'worker' && tab === 'market' && !order.is_my_completed && (
                <button className="btn" onClick={() => takeOrder(order.id)}>Взять в работу</button>
              )}
              {role === 'worker' && tab === 'my_tasks' && !order.is_my_completed && (
                <button className="btn btn-outline" onClick={() => continueOrder(order.id)}>Продолжить ➡</button>
              )}
