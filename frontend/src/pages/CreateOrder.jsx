...
export default function CreateOrder() {
  ...
  const handleUploadDraft = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStep(2); 
      } else alert("Ошибка при загрузке. Возможно, файлы слишком большие.");
    } catch (error) { 
  ...
  const handlePublish = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_URL}/orders/${draftOrderId}/publish`, {
        method: 'POST', 
      ...
      if (res.ok) {
        alert("Опубликовано!"); 
        navigate('/dashboard'); // Возврат в личный кабинет
  ...
  const startHoneypotSetup = () => {
    ...
    setRandomFiles([...serverImages].sort(() => 0.5 - Math.random())); 
    setHpIndex(0); 
  ...
  const saveHoneypotAndNext = (imageId, answer) => {
    setHoneypots(prev => ({ ...prev, [imageId]: answer })); 
    goToNextOrClose();
  };
    ...
    <div className="container" style={{ maxWidth: '900px', paddingBottom: '50px' }}>
      ...
      {step === 2 && (
        <div className="card">
          ...
          <button className="btn btn-primary" onClick={handlePublish} disabled={isLoading} style={{ width: '100%', marginTop: '20px', padding: '15px' }}>
            ✅ Опубликовать проект
          </button>
        </div>
      )}
