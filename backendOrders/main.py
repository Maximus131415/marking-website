...
def get_db_connection():
    return psycopg2.connect(dbname="datamark_db", user="datamark", password="password", host="database")
            ...
            try:
                ...
                with zipfile.ZipFile(temp_zip_path, 'r') as z:
                    for filename in z.namelist():
                        ...
                        saved_images.append({"id": cur.fetchone()[0], "url": f"http://localhost:8002/{file_location}"})
            finally:
...
class PublishData(BaseModel): honeypots: dict
...
def export_results(order_id: int, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for row in cur.fetchall():
            img_id = row[0]
            if img_id not in results_map: results_map[img_id] = {"url": f"http://localhost:8002/{row[1]}", "anns": []}
...
def get_order_images(order_id: int, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return [{"id": row[0], "url": f"http://localhost:8002/{row[1]}", "isDone": row[2]} for row in cur.fetchall()]
    finally: cur.close(); conn.close()